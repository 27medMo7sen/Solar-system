...
export const Signin = () => {
  ...
  useEffect(() => {
    ...
    if (data.status >= 400) {
      dispatch(
        uiActions.toastEmitter({ type: "error", message: data.message })
      );
    } else if (data.ret) {
      dispatch(userActions.login(data.ret));
      dispatch(
        uiActions.toastEmitter({
          type: "success",
          message: "Login successful!",
        })
      );
      localStorage.setItem("first_name", data.ret.first_name);
      localStorage.setItem("last_name", data.ret.last_name);
      localStorage.setItem("email", data.ret.email);
      localStorage.setItem("phone_number", data.ret.phone_number);
      localStorage.setItem("role", data.ret.role);
      if (data.ret.profile_pic)
        localStorage.setItem("profile_pic", data.ret.profile_pic);
      navigate("/");
    }
  }, [data, dispatch]);
      ...
      <Form method="POST">
        <div>
          <label
            className={`${
              !emailHasError
                ? "text-sm font-medium text-gray-700 mb-1 flex gap-1"
                : "text-red-500 flex gap-1 mb-1 text-sm font-medium"
            }`}
          >
        ...
        <div>
          <div className="flex justify-between items-center mb-1">
            <label
              className={`gap-1 text-sm font-medium text-gray-700 flex ${
          ...
          <input
            ...
            className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 ${
              passwordHasError
                ? "animate-shake border-red-500 bg-red-300 focus:bg-red-100"
                : ""
            }`}
            required
          />
