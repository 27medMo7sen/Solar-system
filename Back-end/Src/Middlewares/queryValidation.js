...
// Allowed fields to search in for different schemas
const allowedSearchFields = {
  products: ["title", "desc"],
  users: ["name", "email"],
  blogs: ["title", "author", "content"],
  else: [],
};
  ...
  Joi.object({
    ...
    fields: Joi.string()
      ...
      .custom((value, helpers) => {
        ...
        const validFields = allowedSearchFields[schemaName] || [];

        for (let field of fields) {
          if (!validFields.includes(field.trim())) {
            return helpers.message(`Invalid search field: ${field}`);
          }
        }
        return value;
      })
      .messages({
        ...
        "any.empty": "Fields cannot be empty",
      }),
  });

export const validateQuery = (schemaName = "else") => {
  return (req, res, next) => {
